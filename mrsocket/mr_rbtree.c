...
static inline struct mr_rbtree_node* search(struct mr_rbtree_node* x, uintptr_t key, unsigned int key_len){
	...
	int ret = memcmp((void*)x->key, (void*)key, key_len);
	if (ret == 0)
		return x;

	while(x != NULL){
		ret = memcmp((void*)key, (void*)x->key, key_len);
		if (ret < 0)
			x = x->left;
		else if (ret > 0)
			x = x->right;
...
static void insert_rbtree(struct mr_rbtree_root *root, struct mr_rbtree_node *node){
	...
	while (x != NULL){
		y = x;
		ret = memcmp((void*)node->key, (void*)x->key, key_len);
		if (ret < 0)
			x = x->left;
		else
			x = x->right;
	...
	rb_parent(node) = y;
	if (y != NULL){
		ret = memcmp((void*)node->key, (void*)y->key, key_len);
		if (ret < 0)
			y->left = node;
		else
			y->right = node;
